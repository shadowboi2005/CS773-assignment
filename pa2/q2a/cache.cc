...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if(cache_type == IS_LLC){
            set = get_set_static(MSHR.entry[mshr_index].address,MSHR.entry[mshr_index].cpu);
        }else{
...
if (writeback_cpu == NUM_CPUS)
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        uint32_t set;
        if(cache_type == IS_LLC){
            set = get_set_static(WQ.entry[index].address,WQ.entry[index].cpu);
        }else{
...
if((cache_type == IS_L1I || cache_type == IS_L1D) && reads_ready.size() == 0)
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            uint32_t set ;
            if(cache_type == IS_LLC){
                set = get_set_static(RQ.entry[index].address,RQ.entry[index].cpu);
            }else{
        ...
        void CACHE::handle_prefetch()
        {
            ...
            for (uint32_t i=0; i<MAX_READ; i++) {
                ...
                if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
                {
                    ...
                    uint32_t set;
                    if(cache_type == IS_LLC){
                        set = get_set_static(PQ.entry[index].address,PQ.entry[index].cpu);
                    }else{
        ...
        uint32_t CACHE::get_set_static(uint64_t address, uint32_t cpu)
        {
            ...
            return (uint32_t) ((address & ((1 << lg2(NUM_SET)) - 2)) | cpu); 
        }
        ...
        int CACHE::check_hit(PACKET *packet)
        {
            uint32_t set;
            if(cache_type == IS_LLC){
                set = get_set_static(packet->address,packet->cpu);
            }else{
            ...
            for (uint32_t way=0; way<NUM_WAY; way++) {
                if (block[set][way].valid && (block[set][way].tag == packet_tag)) {
                    ...
                    match_way = way;
                    // assert(packet->cpu?(way >=NUM_WAY/2) : (way < NUM_WAY));
                    DP ( if (warmup_complete[packet->cpu] ) {
