...
uint8_t L2C_BYPASS_KNOB =
    0; // Neelu: Set to 1: Bypass Instructions 2: Bypass Data 3: Bypass All

// #define PUSH_PREFETCHES_FROM_L2_TO_L1	//Neelu: Adding to L1 PQ after
// filling prefetch in L2

// #define CHECK_DATA_HIT_ON_STLB_HIT	//Neelu: Adding to check where the
// corresponding data is present in case of an STLB hit
...
// Neelu: Not sending translation requests to STLB for L1D same page prefetch
// requests #define NO_TRANSLATION_PENALTY_FOR_PREFETCHES
...
void CACHE::handle_fill() {
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (cache_type == IS_LLC) {
      set = get_set_dynamic(MSHR.entry[mshr_index].address,
                            MSHR.entry[mshr_index].cpu);
      // now i have to update cpu access stats, along with the bool miss/hit
      bool is_miss = check_hit(&MSHR.entry[mshr_index]) < 0;
      update_stats(fill_cpu, is_miss);
    } else {
    ...
    // Knob for bypassing only instructions or only data or both -
    // L2C_BYPASS_MODE
    ...
    if (L2C_BYPASS_KNOB == 3) // Bypass both instructions and data
      ...
      cout << "Oops! Looks like you forgot to set the L2C_BYPASS_MODE. Head "
              "over to src/cache.cc to get it done.";
      assert(0);
    }

    if (cache_type == IS_L2C &&
        l2c_bypass) // This is a bypass that does not fill the L2C
    {
    ...
    if ((cache_type == IS_LLC) &&
      ...
      if (cache_type == IS_L2C && MSHR.entry[mshr_index].type == PREFETCH &&
          MSHR.entry[mshr_index].pf_origin_level == FILL_L1) {
        pf_miss_l1++;
      }
      // AGUS
    ...
    if (cache_type == IS_ITLB || cache_type == IS_DTLB ||
      ...
      if (MSHR.entry[mshr_index].data == (UINT64_MAX >> LOG2_PAGE_SIZE)) {
        ...
        //Add to procesed queue to notify L1D about dropped request due to page
    fault if((cache_type == IS_STLB || cache_type == IS_ITLB) &&
    MSHR.entry[mshr_index].l1_pq_index != -1) //@Vishal: Prefetech request from
    L1D prefetcher
        {
        assert(MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D);
        PACKET temp = MSHR.entry[mshr_index];
        temp.data_pa = MSHR.entry[mshr_index].data;
        assert(temp.l1_rq_index == -1 && temp.l1_wq_index == -1);
        temp.read_translation_merged = 0; //@Vishal: Remove this before adding
    to PQ temp.write_translation_merged = 0; if (PROCESSED.occupancy <
    PROCESSED.SIZE) PROCESSED.add_queue(&temp); else assert(0);
        }
        else if(cache_type == IS_STLB &&
    MSHR.entry[mshr_index].prefetch_translation_merged &&
    MSHR.entry[mshr_index].fill_l1d!= -1) //@Vishal:Prefetech request from L1D
    prefetcher
        {
        assert(MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D);
        PACKET temp = MSHR.entry[mshr_index];
    ...
    PROCESSED.SIZE) PROCESSED.add_queue(&temp); else assert(0);
        ...
        else if(cache_type == IS_ITLB && (MSHR.entry[mshr_index].l1_pq_index !=
    -1 || MSHR.entry[mshr_index].prefetch_translation_merged) &&
    MSHR.entry[mshr_index].fill_l1i != -1)
        {
        assert(MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D);
        PACKET temp = MSHR.entry[mshr_index];
    ...
    if (do_fill) {
      ...
      if (cache_type == IS_L1D) {
        ...
        /*	cout << "Inverse Table search key: " << hex <<
           (MSHR.entry[mshr_index].address >> 6) << endl; cout << "Full addr: "
           << hex << (MSHR.entry[mshr_index].address) << endl; map<uint64_t,
           uint64_t>::iterator it = ooo_cpu[fill_cpu].PTW.inverse_table.begin();
            cout << " Inverse table size: " <<
           ooo_cpu[fill_cpu].PTW.inverse_table.size() << " Page table size: " <<
           ooo_cpu[fill_cpu].PTW.page_table.size() << endl; while(it !=
           ooo_cpu[fill_cpu].PTW.inverse_table.end())
            {
            cout << " Inverse Table key: " <<hex << it->first << "  Value: "
           <<hex << it->second << endl; it++;
            }

            it = ooo_cpu[fill_cpu].PTW.page_table.begin();
            while(it != ooo_cpu[fill_cpu].PTW.page_table.end())
            {
            cout << " Page Table key: " << hex << it->first << "  Value: " <<
           hex << it->second << endl; it++;
            }*/
      ...
      if (cache_type == IS_DTLB) {
        ooo_cpu[fill_cpu].L1D.l1d_prefetcher_notify_about_dtlb_eviction(
            MSHR.entry[mshr_index].address << LOG2_PAGE_SIZE, set, way, 0,
            block[set][way].address << LOG2_PAGE_SIZE,
            MSHR.entry[mshr_index].pf_metadata);
      }
#endif
...
void CACHE::handle_writeback() {
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) &&
    ...
    uint32_t set;
    if (cache_type == IS_LLC) {
      set = get_set_dynamic(WQ.entry[index].address, WQ.entry[index].cpu);
      // bool is miss is opposite of hit
      bool is_miss = check_hit(&WQ.entry[index]) < 0;
      update_stats(WQ.entry[index].cpu, is_miss);
    } else {
    ...
    /*	if(cache_type == IS_L1D)
        ...
        for(int temp_index = 0; temp_index < regions_accessed.size();
    temp_index++)
        {
        if(regions_accessed[temp_index] == (WQ.entry[index].address >>
    LOG2_SPATIAL_REGION_SIZE))
        {
        found_region = 1;
        way = 0;
        break;
        ...
        if(found_region == 0)
        {
        total_regions.push_back(WQ.entry[index].address >>
    LOG2_SPATIAL_REGION_SIZE); unique_region_count = total_regions.size();
        regions_accessed.push_back(WQ.entry[index].address >>
    LOG2_SPATIAL_REGION_SIZE); if(regions_accessed.size() >
    IDEAL_SPATIAL_REGIONS)
        {
    ...
    if (cache_type == IS_L2C &&
        (WQ.entry[index].type != PREFETCH_TRANSLATION) &&
        (WQ.entry[index].instruction == 0) &&
        (WQ.entry[index].type != LOAD_TRANSLATION) &&
        (WQ.entry[index].type != PREFETCH_TRANSLATION) &&
        (WQ.entry[index].type != TRANSLATION_FROM_L1D))
      way = 0;
#endif
    ...
    if (way >= 0) { // writeback hit (or RFO hit for L1D)
      ...
      if (cache_type == IS_L1D) { // RFO miss
        ...
        if (do_fill) {
          ...
          // Neelu: Sending DTLB eviction notice to L1D
          if (cache_type == IS_DTLB) {
            ooo_cpu[writeback_cpu]
                .L1D.l1d_prefetcher_notify_about_dtlb_eviction(
                    WQ.entry[index].address << LOG2_PAGE_SIZE, set, way, 0,
                    block[set][way].address << LOG2_PAGE_SIZE,
                    WQ.entry[index].pf_metadata);
          }
#endif
...
void CACHE::handle_read() {
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
      ...
      uint32_t set;
      if (cache_type == IS_LLC) {
        set = get_set_dynamic(RQ.entry[index].address, RQ.entry[index].cpu);
        bool is_miss = check_hit(&RQ.entry[index]) < 0;
        update_stats(RQ.entry[index].cpu, is_miss);
      } else {
      ...
      /*	    if(cache_type == IS_L1D)
         ...
         temp_index++)
              {
              if(regions_accessed[temp_index] == (RQ.entry[index].address >>
         LOG2_SPATIAL_REGION_SIZE))
              {
              found_region = 1;
              way = 0;
              ...
              if(found_region == 0)
              {
              total_regions.push_back(RQ.entry[index].address >>
         LOG2_SPATIAL_REGION_SIZE); unique_region_count = total_regions.size();
              regions_accessed.push_back(RQ.entry[index].address >>
         LOG2_SPATIAL_REGION_SIZE); if(regions_accessed.size() >
         IDEAL_SPATIAL_REGIONS)
              {
              regions_accessed.erase(regions_accessed.begin());
      ...
      /*
         ...
         cout <<  "no" << counter << ","  << hex << RQ.entry[index].full_addr <<
         "," << dec << RQ.entry[index].event_cycle << "," ; if(way >=0) cout <<
         "1" << endl; else cout << "0" << endl; counter++;
         }
         */
      ...
      if (cache_type == IS_DTLB && way < 0) {
        ...
        if (way_pb >= 0) {
          ...
          RQ.entry[index].data_pa =
              ooo_cpu[read_cpu].DTLB_PB.block[0][way_pb].data;
          ooo_cpu[read_cpu].DTLB_PB.sim_hit[read_cpu][RQ.entry[index].type]++;
          ooo_cpu[read_cpu]
              .DTLB_PB.sim_access[read_cpu][RQ.entry[index].type]++;
          ooo_cpu[read_cpu].DTLB_PB.HIT[RQ.entry[index].type]++;
          ooo_cpu[read_cpu].DTLB_PB.ACCESS[RQ.entry[index].type]++;
          ooo_cpu[read_cpu].DTLB_PB.update_replacement_state(
              read_cpu, 0, way_pb,
              ooo_cpu[read_cpu].DTLB_PB.block[0][way_pb].full_addr,
          ...
          way = find_victim(read_cpu, RQ.entry[index].instr_id, set, block[set],
                            RQ.entry[index].ip, RQ.entry[index].full_addr,
                            RQ.entry[index].type);
          // cout << "DTLB_PB hit "<< RQ.entry[index].instr_id << " " <<
          // ooo_cpu[read_cpu].DTLB_PB.pf_useful << endl;
      ...
      if (way >= 0) { // read hit
        ...
        if (cache_type == IS_ITLB) {
          ...
          (RQ.entry[index].l1_rq_index != -1))
              {
              stlb_hints_to_l2++;
              uint64_t phy_addr = ((block[set][way].data << LOG2_PAGE_SIZE) |
          (ooo_cpu[read_cpu].L1D.RQ.entry[RQ.entry[index].l1_rq_index].full_addr
          & ((1 << LOG2_PAGE_SIZE) - 1))) >> LOG2_BLOCK_SIZE;
          //Neelu: Sending type as 6 so that L2C prefetcher can differentiate
          the STLB hints. DP ( if (warmup_complete[read_cpu] ) { cout << "[" <<
          NAME << "] " << __func__ << "sending stlb hint to L2: "; cout << "
          phy_addr: " << hex << phy_addr; cout << " ip: " << RQ.entry[index].ip
          << endl; });

          uint32_t temp_metadata =
          ooo_cpu[read_cpu].L2C.l2c_prefetcher_operate(phy_addr<<LOG2_BLOCK_SIZE,
          RQ.entry[index].ip, 0, 6, 0);

          float l2c_mpki = 0; // =
          (ooo_cpu[fill_cpu].L2C.sim_access[fill_cpu][0]*1000)/(ooo_cpu[fill_cpu].num_retired);
          if(warmup_complete[read_cpu])
          if(ooo_cpu[read_cpu].num_retired -
          ooo_cpu[read_cpu].warmup_instructions > 0) l2c_mpki =
          (ooo_cpu[read_cpu].L2C.sim_miss[read_cpu][0]*1000)/(ooo_cpu[read_cpu].num_retired
          - ooo_cpu[read_cpu].warmup_instructions); else
          if(ooo_cpu[read_cpu].num_retired > 0)
          l2c_mpki =
          (ooo_cpu[read_cpu].L2C.sim_miss[read_cpu][0]*1000)/(ooo_cpu[read_cpu].num_retired);

          uncore.LLC.llc_prefetcher_operate(phy_addr<<LOG2_BLOCK_SIZE,
          RQ.entry[index].ip, 0, 6, temp_metadata, l2c_mpki);
          //ooo_cpu[read_cpu].L2C.l2c_pref_accuracy);
          //	if((((temp_metadata >> 17) & 1) | ((temp_metadata >> 18) & 1))
          == 1)
          //	{
        ...
        if ((mshr_index == -1) &&
          ...
          if (cache_type == IS_LLC) {
            ...
            if (cache_type == IS_L1I || cache_type == IS_L1D) //@Vishal: VIPT
            {
              ...
              //RQ.entry[index].data_pa = (va_to_pa(read_cpu,
              RQ.entry[index].instr_id, RQ.entry[index].full_addr,
              RQ.entry[index].address)); auto it =
              temp_page_table.find(RQ.entry[index].full_addr >> LOG2_PAGE_SIZE);
          ...
          if ((mshr_index == -1) &&
            ...
            //@Vasudha:Perfect DTLB Prefetcher: add to processed queue even on
            //miss
            /*if (cache_type == IS_DTLB && RQ.entry[index].type != PREFETCH)
              {
            //RQ.entry[index].data_pa = (va_to_pa(read_cpu,
            ...
            /*if(cache_type == IS_STLB)
              {
            // Fill level of incoming request and prefetch packet should be same
            else STLB prefetch request(with instruction=1) might get merged with
            DTLB/ITLB, making send_both_tlb=1 due to a msimatch in instruction
            variable. If this happens, data will be returned to           both
            ITLB and DTLB, incurring MSHR miss
            if(MSHR.entry[mshr_index].fill_level == 1 &&
            RQ.entry[index].fill_level == 1)
            {
            if((MSHR.entry[mshr_index].instruction !=
            RQ.entry[index].instruction) && MSHR.entry[mshr_index].send_both_tlb
            == 0)
            {
            MSHR.entry[mshr_index].send_both_tlb = 1;
            }
            if((MSHR.entry[mshr_index].instruction !=
            RQ.entry[index].instruction) && MSHR.entry[mshr_index].type ==
            PREFETCH)
            {
...
void CACHE::handle_prefetch() {
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) &&
      ...
      uint32_t set;
      if (cache_type == IS_LLC) {
        set = get_set_dynamic(PQ.entry[index].address, PQ.entry[index].cpu);
        bool is_miss = check_hit(&PQ.entry[index]) < 0;
        update_stats(PQ.entry[index].cpu, is_miss);
      } else {
...
uint32_t CACHE::get_set_static(uint64_t address, uint32_t cpu) {
  uint32_t set = (address & ((1 << lg2(NUM_SET)) - 2));
  return set | (cpu & 1);
}

uint32_t CACHE::get_set_dynamic(uint64_t address, uint32_t cpu) {
  uint32_t set = (address & ((1 << lg2(NUM_SET)) - 1));
  uint32_t threshold = (NUM_SET * partition_ratio) / 100;

  if (cpu == 0) {
    return set % threshold;
  } else {
    return threshold + (set % (NUM_SET - threshold));
  }
}

void CACHE::update_partition() {
  double miss_rate0 = (double)cpu_misses[0] / cpu_accesses[0];
  double miss_rate1 = (double)cpu_misses[1] / cpu_accesses[1];

  double total_miss_rate = miss_rate0 + miss_rate1;
  if (total_miss_rate > 0) {
    uint32_t new_partition_ratio = (miss_rate0 / total_miss_rate) * 100;

    if (new_partition_ratio > partition_ratio + 10)
      new_partition_ratio = partition_ratio + 10;
    else if (new_partition_ratio < partition_ratio - 10)
      new_partition_ratio = partition_ratio - 10;
    partition_ratio = new_partition_ratio;
  }

  cpu_misses[0] = cpu_misses[1] = 0;
  cpu_accesses[0] = cpu_accesses[1] = 0;
  last_update = current_core_cycle[cpu];
}

void CACHE::update_stats(uint32_t cpu, bool is_miss) {
  cpu_accesses[cpu]++;
  if (is_miss)
    cpu_misses[cpu]++;

  if (current_core_cycle[cpu] - last_update > update_interval) {
    update_partition();
  }
}
...
int CACHE::check_hit(PACKET *packet) {
  uint32_t set;
  if (cache_type == IS_LLC) {
    set = get_set_dynamic(packet->address, packet->cpu);
  } else {
...
int CACHE::invalidate_entry_static(uint64_t inval_addr, uint32_t cpu) {
  uint32_t set = get_set_static(inval_addr, cpu);
  int match_way = -1;
