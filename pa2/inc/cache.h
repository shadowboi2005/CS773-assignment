...
// INICIO AGUS
extern void notify_prefetch(uint64_t addr, uint64_t tag, uint32_t cpu,
                            uint64_t cycle);
// FIN AGUS
...
public:
  ...
  uint64_t pf_requested, pf_issued, pf_useful, pf_useless, pf_fill,
      ...
      pf_lower_level, // Count prefetch request that enters MSHR as new miss(not
                      ...
                      // merged
      pf_lower_level_test, pf_same_fill_level, pf_lower_fill_level, pf_dropped,
      late_prefetch, prefetch_count, unique_region_count, region_conflicts,
      same_page_prefetch_requests, cross_page_prefetch_requests,
      sum_pq_occupancy, pf_pushed_from_L2C, l1d_data_hit, l2c_data_hit,
      llc_data_hit, llc_data_miss, getting_hint_from_l2, sending_hint_to_llc,
      stlb_hints_to_l2, l1d_pref_accuracy, l2c_pref_accuracy,
      instr_evicting_data, // Neelu: data and instr conflicts in L2C
      data_evicting_instr, data_evicting_transl, transl_evicting_data,
      instr_evicting_transl, transl_evicting_instr, data_evicting_data,
      instr_evicting_instr, transl_evicting_transl;

  uint64_t cpu_misses[NUM_CPUS], cpu_accesses[NUM_CPUS];
  uint32_t partition_ratio;
  uint64_t update_interval, last_update;
  uint64_t self_evictions[NUM_CPUS];
  ...
  uint64_t sim_access[NUM_CPUS][NUM_TYPES], sim_hit[NUM_CPUS][NUM_TYPES],
      ...
      sim_instr_miss[NUM_CPUS]
                    [NUM_TYPES], // Neelu: To calculate instruction misses in L2
      roi_access[NUM_CPUS][NUM_TYPES], roi_hit[NUM_CPUS][NUM_TYPES],
      roi_miss[NUM_CPUS][NUM_TYPES], roi_instr_miss[NUM_CPUS][NUM_TYPES];
  ...
  int check_hit(PACKET *packet), invalidate_entry(uint64_t inval_addr),
      invalidate_entry_static(uint64_t inval_addr, uint32_t cpu),
  ...
  void update_partition(), update_stats(uint32_t cpu, bool is_miss);
  ...
  uint32_t get_set(uint64_t address), get_way(uint64_t address, uint32_t set),
      get_set_static(uint64_t address, uint32_t cpu),
      get_set_dynamic(uint64_t address, uint32_t cpu),
